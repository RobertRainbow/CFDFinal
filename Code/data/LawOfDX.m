...
plot(x,p_exact,'LineWidth',2);
legend_entries{1} = 'exact';
for i = 1:length(dx_array)
    x = linspace(-1,1,length(p_array{i}));
    plot(x,p_array{i},'LineWidth',1);
    legend_entries{i+1} = sprintf('dx = %.4f', dx_array(i));
end
legend(legend_entries);
xlabel('x'); ylabel('p');
title('p-solve with dx changing');

figure;
hold on
x = linspace(-1,1,length(rho_exact));
plot(x,rho_exact,'LineWidth',2);
legend_entries{1} = 'exact';
for i = 1:length(dx_array)
    x = linspace(-1,1,length(rho_array{i}));
    plot(x,rho_array{i},'LineWidth',1);
    legend_entries{i+1} = sprintf('dx = %.4f', dx_array(i));
end
legend(legend_entries);
xlabel('x'); ylabel('rho');
title('rho-solve with dx changing');
